...
const InputField = ({ id, type, value, label, error, onChange }) => (
  <div className='flex flex-col justify-end w-full h-full big:text-2xl'>
    <label className='ml-1 mb-1 block text-headings2 font-semibold' htmlFor={id}>
      {label}
    </label>
    <input
      type={type}
...
const CourseSelection = ({ course, title, isOpen, onClose }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      emailjs
        .send(
          "service_mj7nv5d", // Your service ID
          "template_n8fp1wm", // Your template ID
          {
            name: formData.name,
            ...
            courseName: course ? course.name : "No course selected",
          },
          "H3OV5XwOyzlLlqydo" // Your user ID (public key)
        )
        .then(
          (result) => {
  ...
  return (
    <Modal
      ...
      <div className='h-full w-full overflow-y-scroll rounded-lg formBg p-4'>
        <h2 className='formHeader text-center text-lg md:text-xl'>{title}</h2>
